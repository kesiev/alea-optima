...
function SVGTemplate(file,orientation,breakCache) {
...
function SVG(template) {
    ...
    this.getOrientation = () => template.getOrientation();
...
function PDFBook() {
    ...
    this.addPage=(svg)=>{
        pages.push(svg.getSVG());
        orientation = svg.getOrientation();
    }
    ...
    function addPages(doc,id) {
        let page=pages[id];
        if (page) {
            let pdfNode = document.createElement("div");
            if (id>0) doc.addPage();
            pdfNode.innerHTML=page;
            const svgElement=pdfNode.firstElementChild;
            svgElement.getBoundingClientRect();
            doc.svg(svgElement).then(()=>{
                addPages(doc,id+1);
            })
        } else {
            pages=[];
